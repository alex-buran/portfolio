...
A series of original, zero-budget landing page concepts — each one built to showcase a different technical effect (shaders, interactive 3D, scroll-driven motion) wrapped in its own distinct visual design. Built as standalone single-file demos: no build step, no backend, no paid assets.

Each concept lives in its own folder with the source code (`index.html`) and a README explaining what it does. Live demo links below.
